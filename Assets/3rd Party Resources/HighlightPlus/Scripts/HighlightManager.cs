    ...
    public class HighlightManager : MonoBehaviour {
        ...
        void Update() {
            ...
            if (raycastSource == RayCastSource.MousePosition) {
                collision = Physics.Raycast(raycastCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, 10, layerMask);
            } else {
                collision = Physics.SphereCast(Camera.main.transform.position, 0.35f, Camera.main.transform.forward,
                out hitInfo, maxDistance > 0 ? maxDistance : raycastCamera.farClipPlane, layerMask);
            }
